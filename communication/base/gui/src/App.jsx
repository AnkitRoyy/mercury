...
export default function App() {
  const { connected, state, sendCommand } = useRobot()
  ...
  return (
    <div className="h-screen w-screen bg-neutral-950 text-zinc-100 flex flex-col overflow-hidden font-sans">
      ...
      <div className="flex-1 min-h-0 overflow-hidden p-3 flex flex-col gap-3">
        {/* Large camera feeds — most of the screen */}
        <div className="flex-[5] min-h-0 grid grid-cols-2 gap-3">
        ...
        {/* Small bottom row — encoders + servo control */}
        <div className="flex-[1.3] min-h-0 grid grid-cols-2 gap-3">
          <EncoderPanel encoders={state.encoders} />
          <ServoPanel servo={state.servo} sendCommand={sendCommand} />
        </div>
      </div>
    </div>
  )
